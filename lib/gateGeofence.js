...
export function validateGateLocation(location) {
  ...
  if (boundaryDistance > GATE_LOCATION.boundaryBufferMeters) {
    return {
      allowed: false,
      status: 403,
      error: GATE_LOCATION.boundaryBufferMeters > 0
        ? `Gate PO verification is available only within ${GATE_LOCATION.boundaryBufferMeters} m of the approved campus scan area. Your reported position is approximately ${Math.round(boundaryDistance)} m outside it.`
