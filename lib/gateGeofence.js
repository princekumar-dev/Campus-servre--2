...
export function validateGateLocation(location) {
  ...
  if (!isInsideCampusBoundary(latitude, longitude)) {
    return {
      allowed: false,
      status: 403,
      error: `Gate PO verification is available only inside the MSEC campus boundary. Your reported position is approximately ${Math.round(distance)} m from the campus centre.`
    }
  }
